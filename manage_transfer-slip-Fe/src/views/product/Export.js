...
export default function InventoryDetails() {
  const URL_EXPORT = "http://localhost:8080/transfers/export?id="
  // const URL_EXPORT = "http://localhost:8080/transfers/import?id="
  const URL_EXPORT_SIZE = "http://localhost:8080/transfers/export/size?id="
  ...
  function loadSize() {
    ...
    if (search == "") {
      if ((fromDate === null) && (toDate === null)) {
        url = URL_EXPORT_SIZE + getUser.inventoryId
      }
      else {
        if (fromDate != null) {
          if (toDate == null) {
  ...
  function loadData() {
    ...
    if (search == "") {
      if ((fromDate === null) && (toDate === null)) {
        url = URL_EXPORT + getUser.inventoryId + "&page=" + page.number + "&limit=" + page.limit
      }
  ...
  const ConfirmExport = (e) => {
    ...
    var config = {
      method: 'put',
      url: `http://localhost:8080/transfers/shipping/${e.id}`,
      headers: {
        'authorization': `Bearer ${getUser.token}`,
        'Accept': 'application/json',
        'Content-Type': 'application/json'
    ...
    <>
      <CRow>
        <CCol xs="12" sm="12">
          ...
          <div className="mb-6" style={{ color: "#686868" }} > <h2><b>Thông tin xuất hàng kho {getUser.inventoryId}</b></h2> </div>
        </CCol>
      </CRow>
      <CRow>
        <CCol xs="12" sm="12">
                  ...
                  scopedSlots={
                    {
                      ...
                      'confirm':
                        ...
                        (item) => (
                          <td>
                            <span className=" text-primary" style={{ width: "50 %", fontSize: "13px", cursor: "pointer" }} onClick={() => { ConfirmExport(item) }}>Chuyển hàng
                            </span>
                          </td>
                        ),
                    }
